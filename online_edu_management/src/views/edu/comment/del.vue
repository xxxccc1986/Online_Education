      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="primary"
            @click="reportCommentToNormal(scope.row.id,scope.row.username)">
            无违规
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //评论无违规处理
    reportCommentToNormal(commentId,username){
      this.$confirm(`此操作将 ${username} 的评论信息恢复为正常状态, 是否继续?`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
